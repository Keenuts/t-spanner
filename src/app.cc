...
static int compute_linear_greedy(const char *input_file, const char *output_file, float t)
{
  int res;
...
static PyObject* compute_linear_greedy(const char *input_file, float t)
{
  int res;
  struct graph_t output;
  std::memset(&output, 0, sizeof(struct graph_t));
...
static int compute_linear_wspd(const char *input_file, graph_t *output, float t)
{
  int res;
  graph_t *input = new graph_t();
  if (!input)
    return STATUS_ERROR_GRAPH_ALLOC;

  input = new graph_t();
  std::memset(input, 0, sizeof(struct graph_t));

  res = compute_linear_greedy(input_file, output, t);
  if (res) {
    printf("[!] WSPD (linear) returned with error 0x%x\n", res);
    return res;
  }
...
static int compute_linear_wspd(const char *input_file, const char *output_file, float t)
{
  int res;
  struct graph_t output;
  std::memset(&output, 0, sizeof(struct graph_t));

  res = compute_linear_wspd(input_file, &output, t);
  if (res) {
    printf("[!] WSPD (linear) returned with error 0x%x\n", res);
    return res;
  }
...
static PyObject* compute_linear_wspd(const char *input_file, float t)
{
  int res;
  struct graph_t output;
  std::memset(&output, 0, sizeof(struct graph_t));
  ...
  res = compute_linear_wspd(input_file, &output, t);
  if (res) {
    printf("[!] WSPD (linear) returned with error %s(0x%x)\n", error_to_str(res), res);
    //FIXME: Python needs error setting
    return NULL;
  }
  return output_to_python(output);
...
static PyObject* tspanner_greedy(PyObject *self, PyObject *args)
{
  ...
  if (output_file)
    return PyLong_FromLong(compute_linear_greedy(input_file, output_file, t));
  else
    return compute_linear_greedy(input_file, t);
}
*/
...
{                                                             \
  ...
  if (!PyArg_ParseTuple(args, "fs|s", &t, &input_file, &output_file))  \
      return NULL;                                                     \
                                                                       \
  if (output_file)                                                     \
    return PyLong_FromLong(Func(input_file, output_file, t));      \
  else                                                                 \
    return Func(input_file, t);                                    \
}

PY_WRAPPER(greedy, compute_linear_greedy)
PY_WRAPPER(wspd, compute_linear_wspd)
